...
const Clusters = ({ query, accountID }) => {
  ...
  const onProvisionerClicked = (clusterID) => {
    localStorage.setItem('clusterID', clusterID);
    history.push(`/provisioner?account=${accountID}&clusterID=${clusterID}`);
    history.go();
  };
        ...
        return (
          <span className="oneline">
            <img
              ...
              src="https://i.ibb.co/tLktm91/os-icon.png"
            />
            {value}
          </span>
        );
            ...
            {rows.map((row) => (
              <React.Fragment key={row.id}>
                <TableExpandRow {...getRowProps({ row })}>
                  ...
                  <Button
                    renderIcon={Money}
                    iconDescription="Module Provisioner"
                    hasIconOnly
                    kind="tertiary"
                    size="field"
                    type="button"
                    tooltipPosition="right"
                    onClick={() => onProvisionerClicked(clusters.data[row.id].name)}
                  />
                </TableExpandRow>
